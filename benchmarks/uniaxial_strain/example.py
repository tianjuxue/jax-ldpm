...
def bc_disp_control(top_inds_node, bottom_inds_node, wall_inds_node):
    def pre_compute_bc():
        N_btm_ind = len(bottom_inds_node)
        N_tp_ind = len(top_inds_node)
        N_wall_ind = len(wall_inds_node)

        bottom_inds_tiled = np.tile(bottom_inds_node, 6)
        top_inds_tiled = np.tile(top_inds_node, 6) 
        wall_inds_tiled = np.tile(wall_inds_node, 4) 
        inds_node = np.hstack((bottom_inds_tiled, top_inds_tiled, wall_inds_tiled))

        bottom_inds_var = np.repeat(np.array([2, 3, 4, 8, 9, 10]), N_btm_ind)
        top_inds_var = np.repeat(np.array([2, 3, 4, 8, 9, 10]), N_tp_ind)
        wall_inds_var = np.repeat(np.array([0, 1, 6, 7]), N_wall_ind)
        inds_var = np.hstack((bottom_inds_var, top_inds_var, wall_inds_var))

        return N_btm_ind, N_tp_ind, N_wall_ind, inds_node, inds_var

    @partial(jax.jit, static_argnums=(1, 2, 3))
    def crt_bc(step, N_btm_ind, N_tp_ind, N_wall_ind, inds_node, inds_var):
        crt_time = step*dt
        ...
        wall_vals = np.zeros(N_wall_ind*4)

        bc_vals = np.hstack((bottom_vals, top_vals, wall_vals))
...
def simulation():
    ...
    wall_inds_node = np.argwhere((points[:, 0]**2 + points[:, 1]**2)**0.5 > 50. - 0.1).reshape(-1)
    ...
    pre_compute_bc, crt_bc = bc_disp_control(top_inds_node, bottom_inds_node, wall_inds_node)
